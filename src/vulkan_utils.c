...
void new_Instance(                              //
  ...
  // and glfw)
  uint32_t allExtensionCount = enabledExtensionCount + 1;
...
ErrVal getPhysicalDevice(VkPhysicalDevice *pDevice, const VkInstance instance) {
  ...
  if (res != VK_SUCCESS || deviceCount == 0) {
    LOG_ERROR(ERR_LEVEL_WARN, "failed to list physical devices");
    return (ERR_NOTSUPPORTED);
  }
  VkPhysicalDevice *arr = malloc(deviceCount * sizeof(VkPhysicalDevice));
  if (!arr) {
    LOG_ERROR(ERR_LEVEL_FATAL,
              "failed to get physical device: couldn't allocate memory");
    PANIC();
  }
  res = vkEnumeratePhysicalDevices(instance, &deviceCount, arr);
  if (!arr) {
...
void new_RayTracingEnabledDevice(              //
  ...
  const char **ppAllExtensionNames = malloc(allExtensionCount * sizeof(char *));
  if (ppAllExtensionNames == NULL) {
    LOG_ERROR(ERR_LEVEL_FATAL, "couldn't allocate memory");
    PANIC();
  }

  uint32_t index = 0;
  for (uint32_t i = 0; i < enabledExtensionCount; i++) {
    ppAllExtensionNames[index] = ppEnabledExtensionNames[i];
    index++;
  }
  for (uint32_t i = 0; i < raytraceExtensionCount; i++) {
    ppAllExtensionNames[index] = ppRaytraceExtensionNames[i];
    index++;
  }
  ...
  // so we can enable them when we create the device
  vkGetPhysicalDeviceFeatures2(
      physicalDevice, &features2); // enable all the features our GPU has
  ...
  // now that we're done with creating extension names we can delete
  free(ppAllExtensionNames);
}
...
ErrVal new_Swapchain(VkSwapchainKHR *pSwapchain, uint32_t *pImageCount,
  ...
  // we add the image usage so that we can copy the ray traced image to the
  // swapchain fo presentation
